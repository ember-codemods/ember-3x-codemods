...
module.exports = function transformer(file, api) {
  ...
  root
    ...
    .forEach(path => {
      ...
      let idxWillTransition = props.findIndex(p => p.key.name === "willTransition");
      let routeWillChange;
      ...
      if(idxWillTransition >= 0) {
        ...
        routeWillChange = j.expressionStatement(
          j.callExpression(j.memberExpression(j.thisExpression(), j.identifier("on"), false), [
            j.literal("routeWillChange"),
            j.arrowFunctionExpression([j.identifier("transition")], j.blockStatement(wtBody), false)
          ])
        );

      // Cleanup
      props.splice(props.findIndex(p => p.key.name === "willTransition"), 1);
      }

      let idxDidTransition = props.findIndex(p => p.key.name === "didTransition");
      let routeDidChange;
      ...
      if(idxDidTransition >= 0) {
        ...
        routeDidChange = j.expressionStatement(
          j.callExpression(j.memberExpression(j.thisExpression(), j.identifier("on"), false), [
            ...
            j.arrowFunctionExpression([j.identifier("transition")], j.blockStatement(dtBody), false)
          ])
        );

      // Cleanup
      props.splice(props.findIndex(p => p.key.name === "didTransition"), 1);
      }
